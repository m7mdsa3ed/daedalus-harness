...
//
// Local changes to the generated file, both in the effect below:
//   - `prefers-reduced-motion: reduce` freezes time and drops cursor tracking,
//     so it renders one static frame instead of a continuous animation.
//   - the Next-only "use client" directive is gone; this is a Vite app.
...
export function ShaderBackground({
  ...
  className?: string
  /** sRGB 0..1 triplets, darkest first, overriding the built-in ramp. Fewer
      than two entries falls back to the generated palette. */
  colors?: [number, number, number][]
}) {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  // The effect below is deliberately not re-run when the palette changes —
  // rebuilding the GL context to recolour it would flash. The ref lets the
  // live render loop pick up a new ramp on its next frame instead.
  const paletteRef = useRef(colors)
  paletteRef.current = colors
  /* Under reduced motion (or a zero time scale) the loop draws one frame and
     parks, so a palette change would never be picked up. This is how the
     running effect gets told to draw once more. */
  ...
  useEffect(() => {
    ...
    /* Reduced motion turns this into a still: time stops advancing and the
       pointer stops steering, so one frame is drawn and the render loop parks.
       The image stays; only the movement goes. */
    const reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches
    const timeScale = reduceMotion ? 0 : UNIFORMS.timeScale
    const cursorEnabled = UNIFORMS.cursorEnabled && !reduceMotion
    ...
    /* The palette is eight vec3s whatever happens: u_colorCount says how many
       of them the ramp actually walks, and the tail is padded with the last
       colour so an unused slot can never read as black. */
    let appliedPalette = ""
    let colorCount = UNIFORMS.colorCount
    const applyPalette = () => {
      const custom = paletteRef.current
      const ramp = custom && custom.length >= 2 ? custom : UNIFORMS.colors
      const count = custom && custom.length >= 2 ? custom.length : UNIFORMS.colorCount
      const key = `${count}:${ramp.flat().join(",")}`
      if (key === appliedPalette) return
      appliedPalette = key
      colorCount = Math.min(count, 8)
      const padded = Array.from({ length: 8 }, (_, i) => ramp[Math.min(i, ramp.length - 1)])
    ...
    /* An arrow bound to a const, not a `function` declaration: hoisted
       declarations do not inherit the null-narrowing from the `canvas`/`gl`
       guards at the top of the effect, since nothing stops one being called
       before those ran. requestRender above only touches plain locals, so it
       can stay hoisted — and it only *references* this at call time, which is
       always after this line. */
    ...
    nudgeRef.current = requestRender
    requestRender()
    return () => {
      disposed = true
      nudgeRef.current = () => {}
